...
3. **asset catalog: 2 dangling `StaticMesh` refs** — new weapon defs `ID_ArmClub_EQ`, `ID_PotMace_EQ`, `FD_ArmClub_SI`, `FD_PotMace_SI` reference `SM_ArmClub`/`SM_PotMace`, which are **absent from `.assets/StaticMesh.json`** (exporter's asset-registry walk; also absent from the old bundle's catalog). → **4 `missing-asset-ref` issues** produce a **dismissible** drift load-gate ("Continue anyway"; does not block saves). **NOT fixed in-repo**: there is no override mechanism for asset catalogs, and fabricating asset-registry entries or dropping beta data would make the bundle unfaithful to source-of-truth. **This is the sole residual exception to the "no load-gate" acceptance.** Proper fix belongs in `tsic-default-mod@beta` (catalog the 2 meshes, or remove the dangling refs), then re-pin. `smoke:def` fails only because it doesn't dismiss this gate — left unweakened so it keeps surfacing the real signal.

**Merge caution (§4.2):** merging to `main` auto-deploys live. Recommend fixing upstream gap #3 in the mod repo and re-pinning **before** merge, so live users don't see the dismissible drift modal. Committing to the feature branch does not deploy.
