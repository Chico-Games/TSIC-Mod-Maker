...
export function DefinitionsTab() {
  ...
  const folderIssueCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const [k, issues] of issuesByKey) {
      const rec = definitions.get(k);
      if (rec && issues.length > 0) counts[rec.folder] = (counts[rec.folder] ?? 0) + issues.length;
    }
    return counts;
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="def-folder-count">
                        ...
                        {folderIssueCounts[f] ? (
                          <span
                            className="def-folder-issues"
                            title={`${folderIssueCounts[f]} validation issue${folderIssueCounts[f] === 1 ? '' : 's'} in this folder`}
                          >·⚠{folderIssueCounts[f]}</span>
                        ) : null}
                      </span>
                    </li>
                  );
              ...
              {filesInFolder.map((f) => {
                ...
                return (
                  ...
                  >
                    ...
                    <IssueDot issues={issuesByKey.get(f.key) ?? []} />
